/*﻿
 *  Application note: Thermostat with MODBUS via RS485 for AZ-Touch  
 *  for ESP32
 *  Version 1.1
...
void setup() {
  ...
  */
  ...
  //digitalWrite(TFT_LED, HIGH);    // HIGH to turn backlight on - pcb version 01-01-00	
...
void loop() {
 //TS_Point p = waitTouch();
  ...
  if (holdingRegs[DISP_ONOFF]) {
    ...
    //digitalWrite(TFT_LED, LOW);    // LOW to turn backlight off - pcb version 01-01-00
  }
...
void Modbus_processing()
{
  ...
  if ((holdingRegs[BEEPER] > 500) && (holdingRegs[BEEPER] < 4000)) {
    ledcWriteTone(0,holdingRegs[BEEPER]);
     delay(100);
    ledcWriteTone(0,0);
  } 
